...
def adjust_dict_parcellations_masks(data):
    methods = data.keys()
    subjects = data["ROQS"].keys()
    
    for method in methods:
        subjects_list = []
        for subject in subjects:
            subject_data = {"Subject": subject}
            subject_data["Witelson"] = data[method][subject]["Witelson"]
            subject_data["Hofer"] = data[method][subject]["Hofer"]
            subject_data["Chao"] = data[method][subject]["Chao"]
            subject_data["Cover"] = data[method][subject]["Cover"]
...
def adjust_dict_scalar_maps(data):
    ...
    for method in methods:
        ...
        for subject in subjects:
            subject_data = {"Subject": subject}
            subject_data["Wfa"] = data[method][subject][0]
            subject_data["FA"] = data[method][subject][1]
            subject_data["MD"] = data[method][subject][2]
            subject_data["AD"] = data[method][subject][3]
            subject_data["RD"] = data[method][subject][4]
            subjects_list.append(subject_data)
...
# Parcellations statistics
def adjust_dict_parcellations_statistics(data):
    methods = list(data.keys())
    methods_parc = list(data[methods[0]].keys())
    parts = list(data[methods[0]][methods_parc[0]].keys())
    scalars = list(data[methods[0]][methods_parc[0]][parts[0]].keys())
    subjects = list(data[methods[0]][methods_parc[0]][parts[0]][scalars[0]].keys())
    for method in methods:
        subject_list = []
        for subject in subjects:
